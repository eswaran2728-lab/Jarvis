...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 lg:hidden glass border-t border-orion-border">
        ...
        {navItems.map(({ href, icon: Icon, label }) => {
          ...
          return (
            <Link key={href} href={href} className="flex flex-col items-center gap-1 px-3 py-1">
              ...
              <span className={`text-xs ${active ? 'text-orion-blue' : 'text-slate-500'}`}>{label}</span>
            </Link>
          )
        })}
      </div>
