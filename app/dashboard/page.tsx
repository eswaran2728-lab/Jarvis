...
const quickActions = [
  ...
  { label: 'Ask JARVIS', href: '/assistant', icon: MessageCircle, color: 'text-jarvis-blue bg-jarvis-blue/10 border-jarvis-blue/30' },
  ...
  return (
    <AppShell>
      <div className="p-4 md:p-6 max-w-4xl mx-auto w-full">
        ...
        {/* Orb + Voice */}
        <div className="flex items-center justify-between mb-6 glass rounded-2xl border border-jarvis-border p-6">
          <div>
            <p className="text-xs text-jarvis-blue uppercase tracking-widest mb-1">Status</p>
            <p className="text-white font-semibold">Sir, I am ready.</p>
            <p className="text-slate-400 text-sm mt-1">All systems operational.</p>
            {lastSession && (
              <p className="text-slate-400 text-sm mt-2">
                Last session score: <span className="text-jarvis-blue font-bold">{lastSession.score}/100</span>
              </p>
            )}
          </div>
          <div className="flex flex-col items-center gap-6">
            <JarvisOrb size="md" status="idle" />
            <VoiceCommandButton />
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            ...
            { label: 'Reminders', value: pendingReminders.length, color: 'text-jarvis-blue' },
            { label: 'Best Score', value: Math.max(...mockTrainingSessions.map(s => s.score)), color: 'text-green-400' },
          ].map(({ label, value, color }) => (
            <div key={label} className="glass rounded-xl border border-jarvis-border p-3 text-center">
        ...
        <DashboardCard title="Today's Tasks" className="mb-6">
          {pendingTasks.slice(0, 3).map(task => (
            <div key={task.id} className="flex items-center gap-3 py-2 border-b border-jarvis-border last:border-0">
              <div className="w-2 h-2 rounded-full bg-jarvis-blue flex-shrink-0" />
          ...
          <Link href="/tasks" className="text-xs text-jarvis-blue mt-3 block hover:underline">View all tasks →</Link>
        </DashboardCard>

        {/* Training summary */}
        <DashboardCard title="Recent Training" glowing>
          {mockTrainingSessions.slice(0, 3).map(s => (
            <div key={s.id} className="flex items-center gap-3 py-2 border-b border-jarvis-border last:border-0">
          ...
          <Link href="/progress" className="text-xs text-jarvis-blue mt-3 block hover:underline">View full progress →</Link>
        </DashboardCard>
      </div>
    </AppShell>
  )
